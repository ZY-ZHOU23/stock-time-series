...
test_ts_diff  <- as.numeric(test_data$adjusted_diff)
...
rolling_exog <- lapply(exog_vars, function(var) {
  series_exog <- winsorize(as.numeric(train_data[[var]]), lower = 0.1, upper = 0.9)
  combined_exog <- c(series_exog, as.numeric(test_data[[var]]))
...
xreg_train_diff <- as.matrix(train_data %>% select(all_of(exog_vars)))
...
y_last <- tail(train_data$adjusted_diff, 1)
last_train_level <- tail(train_data$adjusted, 1)
...
test_ts_levels <- as.numeric(ts(test_data$adjusted, frequency = 252,
                                 start = c(1, nrow(train_data) + 1)))
df_plot <- tibble(
  date = test_data$date,
